export default {
	...
	SELECT_RESOURCE_VERSIONS: "select u.url, time, filename from ( select t.filename, resource, time from ResourceVersion v inner join VersionType t on t.id = v.type ) ResourcesVersionCounts inner join URLView u on u.resource = ResourcesVersionCounts.resource;",
