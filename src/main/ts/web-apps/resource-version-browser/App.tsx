...
const COLUMN_ORDER = [
    "id", "title", "ssl", "host", "port", "path", "query"
];
...
const toResource = (next: string, state: State, setState: React.Dispatch<React.SetStateAction<State>>) => {
    ...
    switch (index) {
        case 0:
            return <span className="breadcrumb" key={index}>
                <a href={url} onClick={() => toResource(value, state, setState)}>{value}</a>
            </span>;
        case 1:
            return <span className="breadcrumb" key={index}>
...
const cell = (result: Result, key: string, state: State, setState: React.Dispatch<React.SetStateAction<State>>) => {
    switch (key) {
        case "id":
            ...
            return <a href={`#${id}`} onClick={() => toResource(id, state, setState)}>{id}</a>
...
export function App(props: Props) {
    ...
    return <>
        ...
        <form onChange={e => {
            ...
            const sql =
                `select * from
                ResourceSearch${whereClause ? ` where ${whereClause}` : ""}${order ? ` order by \`${order}\`` : ""} limit ${limit} offset ${offset};`;
            ...
            <label>SSL:</label>
            <input name="ssl" type="checkbox"></input>
            <input name="host" placeholder="Host"></input>
            <input name="port" placeholder="Port" type="number"></input>
            <input name="path" placeholder="Path"></input>
        ...
        {resourcePath(state.path) ? (state.times ?
