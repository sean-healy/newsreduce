...
const hostPath = (path: string) => !!path.match(/^[0-9A-F]{24}$/);
...
const toHost = (next: string, state: State, setState: React.Dispatch<React.SetStateAction<State>>) => {
    console.log("toHost");
    ...
    if (prev !== next || !state.times) {
        const url = `${api}/host-times?id=${BigInt(`0x${breadcrumbs[0]}`)}`;
        fetch(url)
...
const toTime = (next: number, state: State, setState: React.Dispatch<React.SetStateAction<State>>) => {
    ...
    if (prev !== next || !state.formats) {
        const url = `${api}/host-formats?id=${BigInt(`0x${breadcrumbs[0]}`)}&time=${breadcrumbs[1]}`
        fetch(url)
            .then(response => response.json())
...
const toFormat = (next: string, state: State, setState: React.Dispatch<React.SetStateAction<State>>) => {
    ...
    if (prev !== next || !state.version ) {
        const version = `${api}/host-version?id=${BigInt(`0x${breadcrumbs[0]}`)}&time=${breadcrumbs[1]}&format=${breadcrumbs[2]}`;
    ...
    switch (index) {
        case 0:
            return <span className="breadcrumb" key={index}>
                <a href={url} onClick={() => toHost(value, state, setState)}>{value}</a>
            </span>;
        case 1:
            return <span className="breadcrumb" key={index}>
...
const cell = (result: Result, key: string, state: State, setState: React.Dispatch<React.SetStateAction<State>>) => {
    switch (key) {
        case "id":
            ...
            return <a href={`#${id}`} onClick={() => toHost(id, state, setState)}>{id}</a>
...
export function App(props: Props) {
    ...
    else if (state.breadcrumbs[0] && !state.times && hostPath(state.path)) toHost(state.breadcrumbs[0], state, setState);
    return <>
        <header>
        <form onChange={e => {
        ...
        {hostPath(state.path) ? (state.times ?
